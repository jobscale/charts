...
object generated {
  def main(args:Array[String]) {
    ...
    try {
        ...
        spark.sql("CREATE TABLE IF NOT EXISTS joyc (key INT, value STRING) USING hive")
        spark.sql("USE tpcds_scale_3000_parquet_non_cbo_joyc")
        ...
        """
        val q39b = ("q39b", q39b_sql)
        val q39a_sql = """--q39a.sql--
        ...
        val q23a_sql = """--q23a.sql--
         ...
         limit 100
        """
        val q23a = ("q23a", q23a_sql)
        val q23b_sql = """--q23b.sql--
        ...
        """
        val q23b = ("q23b", q23b_sql)
        val q24a_sql = """--q24a.sql--
        ...
        """
        val q24a = ("q24a", q24a_sql)
        val q24b_sql = """--q24b.sql--
        ...
        """
        val q24b = ("q24b", q24b_sql)

    val queries: Seq[(String, String)] = Array(q1, q10, q11, q12, q13, q14a, q14b, q15, q16, q17, q18, q19, q2, q20, q21, q22, q23a, q23b, q24a, q24b, q25, q26, q27, q28, q29, q3, q30, q31, q32, q33, q34, q35, q36, q37, q38, q39a, q39b, q4, q40, q41, q42, q43, q44, q45, q46, q47, q48, q49, q5, q50, q51, q52, q53, q54, q55, q56, q57, q58, q59, q6, q60, q61, q62, q63, q64, q65, q66, q67, q68, q69, q7, q70, q71, q72, q73, q74, q75, q76, q77, q78, q79, q8, q80, q81, q82, q83, q84, q85, q86, q87, q88, q89, q9, q90, q91, q92, q93, q94, q95, q96, q97, q98, q99, ss_max)
    queries.size
    spark.conf.get("spark.sql.statistics.fallBackToHdfs")
    spark.sql("SET spark.sql.statistics.fallBackToHdfs=true").collect
    spark.conf.get("spark.sql.statistics.fallBackToHdfs")
    spark.conf.get("spark.sql.autoBroadcastJoinThreshold")
    spark.sql(s"SET spark.sql.autoBroadcastJoinThreshold=${10485760 * 1}").collect
    spark.conf.get("spark.sql.autoBroadcastJoinThreshold")

    import scala.collection.mutable.ListBuffer
    case class QueryRunInfo(id: String, iteration: Int, timeTaken: Long, extraInfo: String)

    var withDF = scala.collection.mutable.Map[String, Array[org.apache.spark.sql.Row]]()
    var withoutDF = scala.collection.mutable.Map[String, Array[org.apache.spark.sql.Row]]()
    val queryIdsToFilter: Array[String] = Array("q23a")
    val numIterations = 1
    var failedQueryIds = new ListBuffer[String]()
    val extraInfo = new ListBuffer[String]()

    var runInfos: Seq[QueryRunInfo] = Seq.empty

    def getTimeTaken[T](f: => T): Tuple2[Long, T] = {
      val start = System.nanoTime()
      val status = f
      val end = System.nanoTime()
      ((end - start) / 1000 / 1000, status )
    }

    def computeTime(query: Tuple2[String, String]): Unit = {
      def runQuery = {
        try {
          val df = spark.sql(query._2)
          df.collect
          true
        } catch {
          case e: Exception =>
            println(s"Error occurred ${e}")
            failedQueryIds += query._1
            false
        }
      }

      println(s"Executing query: ${query._1}")

      for(i <- 1 to numIterations) {
        var results = getTimeTaken(runQuery)
        var status = results._2
        var timeTaken = results._1
        if (status) {
            val run = QueryRunInfo(query._1, i, timeTaken, "spark.sql.executorSideBroadcast.enabled is true")
            runInfos = runInfos :+ run
        }
      }
    }

    import scala.collection.Set
    def filterQueriesfn(queryId: Int): Boolean = {
      true
    }
    val filteredQueries = queries.filter {
      query =>
        val qid = query._1.substring(1, query._1.length) // remove `q` prefixed to query name
        try {
          val id = Integer.parseInt(qid)
          filterQueriesfn(id)
        } catch {
          case _ =>
            // println(s"Unable to parse qid: ${qid}, retying to parse")
            // It would come here for query names like q14a
            val qid1 = qid.substring(0, qid.length - 1)
            try {
              val id1 = Integer.parseInt(qid1)
              filterQueriesfn(id1)
            } catch {
              case _ =>
                // println(s"Unable to parse qid1: ${qid1}")
                false
            }
        }
    }
    filteredQueries.foreach {
      query =>
          computeTime(query)
    }
